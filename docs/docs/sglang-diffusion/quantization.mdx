...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    <tr>
      <td><code>fp8</code> / <code>nvfp4</code> / <code>mxfp4</code> (online quantization)</td>
      <td>Unquantized checkpoint (offline via AMD Quark coming soon)</td>
      <td><code>--quantization &#123;fp8,nvfp4,mxfp4&#125;</code></td>
      <td>Z-Image-Turbo (validated for fp8/mxfp4), MiniMax-H3 (fp8/nvfp4). Others likely work.</td>
      <td>NVFP4: FlashInfer on Blackwell; MXFP4: <code>aiter</code> on ROCm</td>
      <td>NVFP4 requires SM100+ and FlashInfer FP4 kernels. MXFP4 requires ROCm and MI350+ (gfx95x). Weights quantized at load time; activations quantized dynamically.</td>
    </tr>
    <tr>
...
### NVFP4 Online Quantization

Apply FlashInfer NVFP4 quantization to an unquantized BF16/FP16 transformer.
**Requires NVIDIA Blackwell (SM100+) and FlashInfer with FP4 GEMM support.**
...
Like online FP8, MiniMax-H3 keeps its video/audio patch projections, timestep
MLP, and final video/audio heads in FP32. Optional
`--quantization-ignored-layers` patterns keep additional DiT linears in BF16.
Backend selection follows `SGLANG_DIFFUSION_FLASHINFER_FP4_GEMM_BACKEND`
(`trtllm` by default; `cudnn` / `cutlass` are also available).
